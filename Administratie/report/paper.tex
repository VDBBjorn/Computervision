...
Because of the problems with both this systems, we propose to use a detection system based on camera images. These images present a few advantages over a system using lidar or radar. First of all cameras are way cheaper to install. On top of that they see color (whereas lidar only sees gray scale in the infrared spectrum) and are able to gain very high resolution for more distant objects. There are several downsides to using cameras. They need light to work properly. This becomes a problem at night, because cameras can only rely on the headlights of the car. Secondly computer vision systems using camera images can use a lot op CPU power. TODO
