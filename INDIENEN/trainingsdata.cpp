...
void generateLabels(string frameName, vector<bool>& isRoad, int totalBlocks){
	string fnLabels(io::dirData+frameName+io::labelsPostfix);
...
void saveFrameOutput(const string frameName, const Mat& frame, const Mat& featureVectors, vector<bool>& isRoad, bool useColor, bool useLBP){
	...
	io::checkDir(io::dirData);

	/* Open new CSV-files for frame data in output directory */
	ofstream fosLbl, fosFv;
	string fnLbl = io::dirData+frameName+io::labelsPostfix;
	string fnFv = io::dirData+frameName+(useLBP?io::lbpStr:"")+(useColor?io::colorStr:"")+io::featVecsPostfix;
...
void parameterIterationTraining(bool relabel=false){
	cout<<"Start parameter iteration training"<<endl;
    ...
    cout<<"Preparing trainingsdata..."<<endl;
	...
	bool useLBP = useLBPVals[fvTypeIdx], useColor = useColorVals[fvTypeIdx];
	    ...
	    set<int>& trainingsSet = trainingsSets[tSIdx];
			...
			for(int fIdx=0;fIdx<frameStopIdx;fIdx+=frameInterval){
				...
				if(isTrainingSet)
				    io::readTrainingsdata(frameName,initLabels,initTrainingsdata,useLBP,useColor);
				if(!isTrainingSet || datasetDoubles)
				    io::readTrainingsdata(frameName,testLabels,testTrainingsdata,useLBP,useColor);
			}
			}
	    	// Init SVM and train
